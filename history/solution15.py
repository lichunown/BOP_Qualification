...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun  5 19:11:50 2017
...
qst_input = Input(shape=(200, 400),dtype = 'float32',name = 'qst_input')
ans_input = Input(shape=(200, 400),dtype = 'float32',name = 'ans_input')


qst_x = Conv1D(16, (5, 5), activation='sigmoid')(qst_input)
#qst_x = MaxPooling2D(pool_size=(2, 2))(qst_x)
#qst_x = Dropout(0.25)(qst_x)
qst_x = Flatten()(qst_x)

ans_x = Conv1D(16, (5, 5), activation='sigmoid')(ans_input)
...
def testModel(model=cnn_model,data = devData,n = 1):
    x,y = next(yieldData3_1(devData,n))
...
def testAllsu(cnn_model = cnn_model,data = devData,n=1):
    succeednum = 0
    testn = 0
    yielddatas = yieldData3_1(devData,1)
...
def train(per_traindata_num=50,all_iter=20,testdatas=50,loads_weight=True):
    yielddatas = yieldData3_1(trainData,20)
    if loads_weight:
        try:
